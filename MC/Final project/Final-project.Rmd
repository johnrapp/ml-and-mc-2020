...
norm_points <- rmvnorm(100, mean_size, sigma_size)
rect(rep())
```
...
state = list(
  ...
  # )
  points = list(
    list(x = clusters$centers[1,1], y = clusters$centers[1,2], w = ww, h = hh),
    list(x = clusters$centers[2,1], y = clusters$centers[2,2], w = ww, h = hh)
  )
)
...
propose <- function(state) {
  ...
  if (runif(1) < 0.9) {
    ...
    # state$points[[i]]$x <- norm[1]
    # state$points[[i]]$y <- norm[2]
    norm <- rmvnorm(1, c(state$points[[i]]$x, state$points[[i]]$y), diag(c((w*.005)^2, (h*.005)^2)))
...
plot(ls, col="black", type="l", ylim=c(0,max(c(ls, ps))))
lines(ps, col="red")
# lines(ls / ps, col="gray")
  ...
  for (i in 1:s$k) {
    point <- s$points[[i]]
    rect((point$x - point$w/2) / w, 1-(point$y - point$h/2) / h, (point$x + point$w/2)/w, 1-(point$y + point$h/2)/h, col = "blue")
  }
}
for (s in states[order(ls, decreasing = 1)[c(seq(1,15))]]) {
  plot(0:1,0:1,type="n",ann=FALSE,axes=FALSE)
  rasterImage(foreground,0,0,1,1)
...
for (s in states[order(ls, decreasing = 1)[c(length(states)-seq(1,15)-1)]]) {
  plot(0:1,0:1,type="n",ann=FALSE,axes=FALSE)
  rasterImage(foreground,0,0,1,1)

  for (i in 1:s$k) {
    point <- s$points[[i]]
    rect((point$x - point$w/2) / w, 1-(point$y - point$h/2) / h, (point$x + point$w/2)/w, 1-(point$y + point$h/2)/h, col = "green")
  }
}
for (s in states[sample(1:length(states), 15)]) {
  plot(0:1,0:1,type="n",ann=FALSE,axes=FALSE)
  rasterImage(foreground,0,0,1,1)

  for (i in 1:s$k) {
    point <- s$points[[i]]
    rect((point$x - point$w/2) / w, 1-(point$y - point$h/2) / h, (point$x + point$w/2)/w, 1-(point$y + point$h/2)/h, col = "purple")
  }
}
```
